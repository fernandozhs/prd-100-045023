...
def UpdateFrequencies(fieldRecord, modeRecord, metricRecord, Box):
    ...
    WSample = PhaseIntegralW(xSample, np.sqrt(fSample), xSample, Order=2, N=-1) #Alternatively: N=len(xSample)
    VSample = PhaseIntegralV(xSample, np.sqrt(fSample), xSample, Order=2, N=-1) #Alternatively: N=len(xSample)
...
def JumpForward(fieldRecord, modeRecord, metricRecord, Box, Root, f, rho):
...
def EvolveSystem(metricRecord, fieldRecord, Box, xEnd, Increment, Save=True, Verbose=True, Backreaction=True):
    ...
    # Once the iteration process is complete, a copy of the final iterations of `metricRecordIteration` and `fieldRecordIteration` are stored in `metricRecord` and `fieldRecord`.
